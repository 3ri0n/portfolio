...
if(storedTheme){ root.setAttribute('data-theme', storedTheme); if(themeSwitch) themeSwitch.checked = storedTheme === 'light'; }
...
function print(html, typed=false){
  ...
  if(!typed){ d.innerHTML = html; out.appendChild(d); out.scrollTop = out.scrollHeight; return; }
  out.appendChild(d);
  let i=0; (function tick(){ d.innerHTML = html.slice(0, ++i); out.scrollTop = out.scrollHeight; if(i < html.length) requestAnimationFrame(tick); })();
}
...
input.addEventListener('keydown', e=>{
  ...
  if(val==='clear'){ out.innerHTML=''; input.value=''; return; }
  if(val==='play' || val==='game'){ openGame(); print(`<span class="muted">Launching minigame… Press <b>Esc</b> to exit.</span>`, true); input.value=''; return; }
  if(blocks[val]){ print(`<span class="muted">${blocks[val]}</span>`, true); input.value=''; return; }
  print(`<span class="muted">Command '${val}' not found. Try 'help'.</span>`, true); input.value='';
});
...
(function(){
  ...
  (function loop(){ x+=(tx-x)*0.15; y+=(ty-y)*0.15; const a=Math.atan2(ty-y,tx-x)*180/Math.PI; el.style.transform=`translate(${x}px, ${y}px) rotate(${a/18}deg)`; requestAnimationFrame(loop); })();
  function size(){ el.classList.toggle('small', innerWidth<640); } size(); addEventListener('resize', size);
  const inEl = $('#in');
...
const player = {x:canvas.width/2, y:canvas.height-60, w:44, h:18, vx:0, speed:6};
let obstacles=[], orbs=[], score=0, lastSpawn=0, lastOrb=0;
function rnd(min,max){ return Math.random()*(max-min)+min; }
function resetGame(){ player.x=canvas.width/2; player.vx=0; obstacles=[]; orbs=[]; score=0; lastSpawn=0; lastOrb=0; }
function spawnObstacle(){ const w=rnd(40,110); obstacles.push({x:rnd(0,canvas.width-w), y:-20, w, h:14, vy:rnd(2.2,4.6)}); }
function spawnOrb(){ orbs.push({x:rnd(16, canvas.width-16), y:-20, r:8, vy:rnd(2.4,4.2)}); }
function update(ts){
  ...
  ctx.fillStyle = getComputedStyle(root).getPropertyValue('--accent'); ctx.fillRect(player.x, player.y, player.w, player.h); ctx.fillRect(player.x+10, player.y-6, player.w-20, 6);
...
function openGame(){ modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); resetGame(); running=true; paused=false; raf=requestAnimationFrame(update); }
function closeGame(){ cancelAnimationFrame(raf); running=false; paused=false; modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); }
